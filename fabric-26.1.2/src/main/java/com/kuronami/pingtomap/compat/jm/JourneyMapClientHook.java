...
public final class JourneyMapClientHook {
    ...
    public static void onPingReceived(PingLocationS2CPacket packet) {
        ...
        try {
            ...
            PingToMapFabric.LOGGER.warn("JourneyMap ping waypoint show failed: {}", t.toString());
        }
    }
            ...
            while (it.hasNext()) {
                ...
                if (e.getValue().expireAtMillis <= now) {
                    try {
                        ...
                        PingToMapFabric.LOGGER.debug("ping waypoint removal failed: {}", t.toString());
                    }
                    it.remove();
                }
            }
        ...
        synchronized (TRACKED) {
            for (ScheduledRemoval r : TRACKED.values()) {
                try {
                    ...
                    PingToMapFabric.LOGGER.debug("ping waypoint clearAll removal failed: {}", t.toString());
                }
            }
            TRACKED.clear();
        }
    ...
    private static int resolveLifetimeSec() {
        if (Config.SYNC_WITH_PING_WHEEL.get()) {
            try {
                ...
                PingToMapFabric.LOGGER.debug("Ping-Wheel pingDuration を読めず手動寿命にフォールバック: {}", t.toString());
            }
        }
    ...
    private static final class Inner {
        static void show(PingLocationS2CPacket packet) {
            ...
            if (api == null) {
                PingToMapFabric.LOGGER.debug("JourneyMap API not yet initialized, skipping ping waypoint");
                return;
            }
            ...
            wp.setColor(color);
            api.addWaypoint(PingToMapFabric.MODID, wp);
            ...
            PingToMapFabric.LOGGER.info("Ping waypoint registered: {} @ {} (color=0x{}, lifetime={}s)",
        ...
        static void remove(String waypointGuid) {
            ...
            journeymap.api.v2.common.waypoint.Waypoint wp = api.getWaypoint(PingToMapFabric.MODID, waypointGuid);
            if (wp != null) {
                api.removeWaypoint(PingToMapFabric.MODID, wp);
            }
        }
        ...
        private static void removePrevious(UUID author) {
            ...
            try {
                ...
                PingToMapFabric.LOGGER.debug("previous ping waypoint removal failed: {}", t.toString());
            }
        }
        ...
        private static Integer resolveTeamColor(UUID authorId) {
            ...
            // MC 26.1.2: PlayerTeam#getColor() は ChatFormatting を返す (26.2 で Optional<TeamColor> に変更、javap 確認済み)
